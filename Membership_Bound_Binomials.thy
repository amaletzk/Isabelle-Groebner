...
proof (rule, rule)
  ...
  proof
    ...
    also have "... = s" unfolding g punit.lt_binomial[OF obd] ..
...
lemma in_ideal_2I: "q1 * f1 + q2 * f2 \<in> ideal {f1, f2::'a::comm_powerprod \<Rightarrow>\<^sub>0 'b::ring_1}"
  by (rule ideal.module_closed_plus, rule ideal.module_closed_smult, rule ideal.generator_in_module,
      simp, rule ideal.module_closed_smult, rule ideal.generator_in_module, simp)

lemma in_ideal_2E:
  assumes "f \<in> ideal {f1, f2}"
  obtains q1 q2 where "f = q1 * f1 + q2 * f2"
  using assms
proof (induct f arbitrary: thesis rule: ideal.module_induct)
  case base: module_0
  show ?case
  proof (rule base)
...
lemma in_pideal_2_iff:
  "f \<in> ideal {f1, f2} \<longleftrightarrow> (\<exists>q1 q2. f = q1 * f1 + q2 * (f2::'a::comm_powerprod \<Rightarrow>\<^sub>0 'b::ring_1))"
proof
  assume "f \<in> ideal {f1, f2}"
  then obtain q1 q2 where "f = q1 * f1 + q2 * f2" by (rule in_ideal_2E)
...
next
  ...
  show "f \<in> ideal {f1, f2}" unfolding f_eq by (rule in_ideal_2I)
qed
...
definition vect_p :: "(('x point) * ('x point)) \<Rightarrow> ('x point)"
...
definition poly_point :: "(('x \<Rightarrow>\<^sub>0 nat) \<Rightarrow>\<^sub>0 'b::zero) \<Rightarrow> ('x point * 'x point)" where
...
definition overlap :: "(('x \<Rightarrow>\<^sub>0 nat) \<Rightarrow>\<^sub>0 'b) \<Rightarrow> (('x \<Rightarrow>\<^sub>0 nat) \<Rightarrow>\<^sub>0 'b::zero) \<Rightarrow> ('x \<Rightarrow>\<^sub>0 nat)"
  where "overlap q1 q2 = to_nat_pm (overlap_p (poly_point q1) (poly_point q2))"

definition vect :: "(('x \<Rightarrow>\<^sub>0 nat) \<Rightarrow>\<^sub>0 'b::zero) \<Rightarrow> ('x \<Rightarrow>\<^sub>0 int)"
  where "vect q = to_int_pm (vect_p (poly_point q))"

lemma vect_alt: "vect q = (of_nat_pm (lp q)) - ((of_nat_pm (tp q))::'x \<Rightarrow>\<^sub>0 int)"
...
lemma associated_alt_int:
  "associated q s t k \<longleftrightarrow> (of_nat_pm s = ((of_nat_pm t)::'x \<Rightarrow>\<^sub>0 int) + (int k) \<cdot> (vect q))" (is "?L \<longleftrightarrow> ?R")
proof
  assume ?L
...
lemma of_nat_fun_overlap_leqI:
  ...
  shows "lookup (of_nat_pm (overlap f1 f2)) \<le> lookup (p::'x point)"
  using assms
...
proof -
  ...
  then obtain q1 q2 where g_eq: "g = q1 * f1 + q2 * f2" by (rule in_ideal_2E)
  ...
  proof
    ...
    proof -
      ...
      proof (rule punit.monomial_1_in_pmdlI[simplified])
        show "q1' * f1 + ?q2 * f2 \<in> ideal {f1, f2}" by (rule in_ideal_2I)
      qed
    qed
...
proof -
  ...
  with _ f1_pbinomial thm_3_2_1_aux_1' * have "overlap f1 f2 adds overlapshift (lp g)"
