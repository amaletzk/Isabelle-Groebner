...
proof (rule, rule)
  ...
  proof
    ...
    with \<open>g \<in> pideal F\<close> have "g - monomial c s \<in> pideal F" by (rule ideal.module_closed_minus)
...
lemma in_pideal_2I: "q1 * f1 + q2 * f2 \<in> pideal {f1, f2::'a::comm_powerprod \<Rightarrow>\<^sub>0 'b::ring_1}"
...
lemma in_pideal_2_iff:
  "f \<in> pideal {f1, f2} \<longleftrightarrow> (\<exists>q1 q2. f = q1 * f1 + q2 * (f2::'a::comm_powerprod \<Rightarrow>\<^sub>0 'b::ring_1))"
proof
