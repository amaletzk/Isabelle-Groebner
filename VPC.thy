...
lemma in_idealI_2: "q1 * f1 + q2 * f2 \<in> ideal {f1, f2::'a::comm_powerprod \<Rightarrow>\<^sub>0 'b::ring_1}"
      ...
      proof (cases "l = t''")
        ...
        proof
          ...
          moreover have "f - c0 \<cdot> h \<in> ideal {f1, f2}"
            by (simp add: ideal.module_closed_minus less.prems(1) in_idealI_2 scalar_eq_times
                ideal.module_closed_smult h_def)
          ...
          hence "(1 / lookup f t') \<cdot> (monomial (lookup f t') t') \<in> ideal {f1, f2}"
            by (simp add: ideal.module_closed_smult scalar_eq_times)
        ...
        proof (cases "l' = t''")
          ...
          proof (rule less.prems)
            ...
            proof
              ...
              moreover have "f - c0 \<cdot> h' \<in> ideal {f1, f2}"
                by (simp add: ideal.module_closed_minus less.prems(1) in_idealI_2 scalar_eq_times
                    ideal.module_closed_smult h'_def)
              ultimately have "monomial (lookup f t) t \<in> ideal {f1, f2}" by (simp add: 1 binomial_def)
              hence "(1 / lookup f t) \<cdot> (monomial (lookup f t) t) \<in> ideal {f1, f2}"
                by (simp add: ideal.module_closed_smult scalar_eq_times)
              with \<open>t \<in> keys f\<close> have "monomial 1 t \<in> ideal {f1, f2}"
          ...
          proof (cases "l' = t")
            ...
            next
              show "keys (c0 \<cdot> except q1 (fst Y')) \<subseteq> keys q1"
                by (simp add: Diff_subset keys_except keys_scalar)
            next
