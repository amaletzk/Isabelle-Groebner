...
proof -
  ...
  have "?l = ko_ntm.lookup_dflt (map_pair ?f xs') (term_of_pair (t, i + k))"
  ...
  also have "... = ko_ntm.lookup_dflt (map_pair ?f xs') (fst (?f (term_of_pair (t, k), b)))"
    by (simp add: map_component_term_of_pair)
  also have "... = snd (?f (term_of_pair (t, k), ko_ntm.lookup_dflt xs' (term_of_pair (t, k))))"
