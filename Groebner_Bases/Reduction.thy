...
theory Reduction
imports "../Polynomials/MPoly_Type_Class_Ordered" Groebner_Bases.Confluence
begin
...
subsubsection \<open>Function \<open>rd\<close>\<close>

context ordered_term
begin

function rd_mult :: "('t \<Rightarrow>\<^sub>0 'b::field) \<Rightarrow> ('t \<Rightarrow>\<^sub>0 'b) \<Rightarrow> ('b \<times> 'a) option" where
  "rd_mult p f =
    (if p = 0 then
      None
    else
      (if lt f adds\<^sub>t lt p then
  ...
  by auto
termination proof -
  let ?R = "{(x, y::'t \<Rightarrow>\<^sub>0 'b). keys x \<subset> keys y} <*lex*> {}"
  show ?thesis
  proof
    show "wf ?R"
    proof
...
definition rd::"('t \<Rightarrow>\<^sub>0 'b::field) \<Rightarrow> ('t \<Rightarrow>\<^sub>0 'b) \<Rightarrow> (('t \<Rightarrow>\<^sub>0 'b) \<times> bool)"
  where "rd p f =
          (if f = 0 then
            (p, False)
          else
...
lemma rd_mult_adds:
  assumes "p \<noteq> 0" and "lt f adds\<^sub>t lt p"
  shows "rd_mult p f = Some (lc p / lc f, lp p - lp f)"
  using assms by simp

lemma rd_mult_nadds:
  assumes "p \<noteq> 0" and "f \<noteq> 0" and "\<not> lt f adds\<^sub>t lt p"
...
lemma rd_adds:
  assumes "p \<noteq> 0" and "f \<noteq> 0" and "lt f adds\<^sub>t lt p"
...
lemma fst_rd_nadds:
  assumes "p \<noteq> 0" and "f \<noteq> 0" and "\<not> lt f adds\<^sub>t lt p"
  shows "fst (rd p f) = (monomial (lc p) (lt p)) + fst (rd (tail p) f)"
  by (simp add: fst_rd rd_mult_nadds[OF assms] assms(2) case_distrib[of "(+) (monomial (lc p) (lt p))"]
...
next
  ...
  proof (cases "\<exists>f\<in>{f}. f \<noteq> 0 \<and> lt f adds\<^sub>t lt p")
    assume "\<exists>f\<in>{f}. f \<noteq> 0 \<and> lt f adds\<^sub>t lt p"
    hence "f \<noteq> 0" and "lt f adds\<^sub>t lt p" by auto
    have "red {f} p (p - monom_mult (lc p / lc f) (lp p - lp f) f)"
      by (intro red_indI1, simp, fact+)
    with \<open>f \<noteq> 0\<close> show ?thesis using rd_mult_adds[OF \<open>p \<noteq> 0\<close> \<open>lt f adds\<^sub>t lt p\<close>] by (simp add: rd_def)
  next
    assume "\<not> (\<exists>f\<in>{f}. f \<noteq> 0 \<and> lt f adds\<^sub>t lt p)"
    from this is_red_indE[OF tail(3)] have r: "is_red {f} (tail p)"
      and dis: "f = 0 \<or> \<not> (lt f adds\<^sub>t lt p)" by auto
    from is_red_singleton_not_0[OF r] have "f \<noteq> 0" .
    from dis this have "\<not> (lt f adds\<^sub>t lt p)" by simp
...
proof (induct p rule: poly_mapping_tail_induct, simp add: rd_left0)
  ...
  have "f \<in> {f}" by simp
  from irred is_red_indI1[OF this _ \<open>p \<noteq> 0\<close>] have dis: "f = 0 \<or> \<not> (lt f adds\<^sub>t lt p)" by auto
  show "fst (rd p f) = p"
  proof (cases "f = 0")
    case True
  ...
  next
    case False
    hence nadds: "\<not> (lt f adds\<^sub>t lt p)" using dis by simp
...
lemma fst_rd_less_eq: "fst (rd p f) \<preceq>\<^sub>p p"
...
lemma fst_rd_lessE:
  assumes "fst (rd p f) \<prec>\<^sub>p p"
...
proof (simp add: rd_def del: rd_mult.simps split: option.split, intro allI impI)
  ...
  next
    ...
    qed
  qed
  with \<open>f \<noteq> 0\<close> show "monom_mult c t f \<noteq> 0" by (simp add: monom_mult_eq_zero_iff)
qed

corollary snd_rd_iff_is_red: "snd (rd p f) \<longleftrightarrow> (is_red {f} p)"
  by (simp add: snd_rd_iff_neq fst_rd_id_set)

subsubsection \<open>Functions \<open>rd_list\<close> and \<open>trd\<close>\<close>

primrec rd_list::"('t \<Rightarrow>\<^sub>0 'b::field) list \<Rightarrow> ('t \<Rightarrow>\<^sub>0 'b) \<Rightarrow> ('t \<Rightarrow>\<^sub>0 'b) \<times> bool" where
...
lemma fst_rd_list_le: "fst (rd_list fs p) \<preceq>\<^sub>p p"
...
lemma fst_rd_list_in_pmdl_ind:
  assumes "set fs \<subseteq> bs"
  shows "p - fst (rd_list fs p) \<in> pmdl bs"
  using assms
proof (induct fs)
  from pmdl.module_0 show "p - fst (rd_list [] p) \<in> pmdl bs" by simp
next
  fix a fs
  assume IH: "set fs \<subseteq> bs \<Longrightarrow> p - fst (rd_list fs p) \<in> pmdl bs" and a: "set (a # fs) \<subseteq> bs"
  from a have "a \<in> bs" by simp
  from a have "set fs \<subseteq> bs" by simp
  show "p - fst (rd_list (a # fs) p) \<in> pmdl bs" unfolding rd_list_rec Let_def
  proof (simp add: if_splits, rule, intro impI)
    assume "snd (rd p a)"
    hence "fst (rd p a) \<noteq> p" by (simp add: fst_rd_id_set snd_rd_iff_is_red)
    hence "fst (rd p a) \<prec>\<^sub>p p" using fst_rd_less_eq[of p a] by simp
    from fst_rd_lessE[OF this] obtain t where "red_single p (fst (rd p a)) a t" .
    hence eq: "p - fst (rd p a) = monom_mult (lookup p (t \<oplus> lt a) / lc a) t a"
      unfolding red_single_def by simp
    show "p - fst (rd p a) \<in> pmdl bs" unfolding eq by (rule monom_mult_in_pmdl, rule \<open>a \<in> bs\<close>)
  next
    show "\<not> snd (rd p a) \<longrightarrow> p - fst (rd_list fs p) \<in> pmdl bs"
    proof
      assume "\<not> snd (rd p a)"
      from IH[OF \<open>set fs \<subseteq> bs\<close>] show "p - fst (rd_list fs p) \<in> pmdl bs" .
    qed
  qed
qed

lemma fst_rd_list_in_pmdl: "p - fst (rd_list fs p) \<in> pmdl (set fs)"
  by (rule fst_rd_list_in_pmdl_ind, simp)
...
definition trd_term :: "('a \<Rightarrow> nat) \<Rightarrow> ((('t \<Rightarrow>\<^sub>0 'b::field) list \<times> ('t \<Rightarrow>\<^sub>0 'b)) \<times> (('t \<Rightarrow>\<^sub>0 'b) list \<times> ('t \<Rightarrow>\<^sub>0 'b))) set"
  where "trd_term d = {(x, y). dgrad_p_set_le d (set (snd x # fst x)) (set (snd y # fst y)) \<and> snd x \<prec>\<^sub>p snd y}"

lemma trd_term_wf:
  assumes "dickson_grading (+) d"
  shows "wf (trd_term d)"
proof (rule wfI_min)
  fix x :: "('t \<Rightarrow>\<^sub>0 'b::field) list \<times> ('t \<Rightarrow>\<^sub>0 'b)" and Q
  assume "x \<in> Q"
  ...
  ultimately obtain z0 where "z0 \<in> snd ` ?Q"
    and *: "\<And>y. y \<prec>\<^sub>p z0 \<Longrightarrow> y \<notin> snd ` ?Q" by (rule ord_p_minimum_dgrad_p_set, blast)
  ...
  proof
    ...
    proof (intro allI impI)
      ...
      hence b: "dgrad_p_set_le d (set (snd y # fst y)) (set (snd z # fst z))" and "snd y \<prec>\<^sub>p z0"
        by (simp_all add: trd_term_def z0)
      from \<open>snd y \<prec>\<^sub>p z0\<close> have "snd y \<notin> snd ` ?Q" by (rule *)
...
function trd :: "('t \<Rightarrow>\<^sub>0 'b::field) list \<Rightarrow> ('t \<Rightarrow>\<^sub>0 'b) \<Rightarrow> ('t \<Rightarrow>\<^sub>0 'b)"
...
termination proof -
  ...
  proof (rule, rule trd_term_wf, fact)
    fix p fs and q::"('t \<Rightarrow>\<^sub>0 'b) \<times> bool"
    ...
    proof (simp add: trd_term_def, rule)
      ...
      qed fact
    next
      from red show "fst q \<prec>\<^sub>p p" by (rule red_ord)
    qed
  qed
qed
...
proof (induct p rule: trd.induct)
  fix fs and p::"'t \<Rightarrow>\<^sub>0 'b"
  let ?x = "rd_list fs p"
...
proof (induct p rule: trd_induct)
  fix fs and p::"'t \<Rightarrow>\<^sub>0 'b"
  from pmdl.module_0 show "p - p \<in> pmdl (set fs)" by simp
next
  fix fs and p::"'t \<Rightarrow>\<^sub>0 'b"
  assume IH: "(fst (rd_list fs p) - trd fs (fst (rd_list fs p))) \<in> pmdl (set fs)"
  from pmdl.module_closed_plus[OF IH fst_rd_list_in_pmdl[of p fs]]
    show "p - trd fs (fst (rd_list fs p)) \<in> pmdl (set fs)" by simp
qed
