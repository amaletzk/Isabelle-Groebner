...
sublocale linordered_field \<subseteq> div_semiring 1 "op *" "op \<le>" "op <" "op +" 0 "op /" "op /"
...
(*
subsection \<open>Type @{typ nat}\<close>
  
instantiation nat :: div_semiring
begin

definition div_ceil_nat :: "nat \<Rightarrow> nat \<Rightarrow> nat" where
  "div_ceil_nat m n = (LEAST k. m \<le> n * k)"
definition div_floor_nat :: "nat \<Rightarrow> nat \<Rightarrow> nat" where
  "div_floor_nat m n = m div n"

lemma leq_mult_nat: "n \<noteq> (0::nat) \<Longrightarrow> m \<le> n * m"
  by auto

lemma div_floor_nat_rat:
  assumes "n \<noteq> (0::nat)"
  shows "div_floor m n = to_nat ((of_nat m) / (of_nat n))"
  unfolding div_floor_nat_def to_nat_def using assms by (simp add: floor_divide_of_nat_eq)

lemma div_ceil_nat_rat:
  assumes "n \<noteq> (0::nat)"
  shows "div_ceil m n = nat \<lceil>(of_nat m) / (of_nat n)\<rceil>"
  unfolding div_ceil_nat_def using assms sorry

instance proof
  fix m n :: nat
  assume "0 < n"
  hence "n \<noteq> 0" by simp
  from leq_mult_nat[OF this] show "m \<le> n * div_ceil m n" unfolding div_ceil_nat_def by (rule LeastI)
next
  fix m n k :: nat
  assume  "m \<le> n * k"
  thus "div_ceil m n \<le> k" unfolding div_ceil_nat_def by (rule Least_le)
next
  fix m n :: nat
  assume "n < 0"
  thus "n * div_ceil m n \<le> m" by simp
next
  fix m n k :: nat
  assume "n < 0"
  thus "div_ceil m n \<le> k" by simp
next
  fix m n :: nat
  assume "0 < n"
  thus "n * div_floor m n \<le> m" unfolding div_floor_nat_def using split_div_lemma[OF \<open>0 < n\<close>] by blast
next
  fix m n k :: nat
  assume "0 < n" and "n * k \<le> m"
  thus "k \<le> div_floor m n" unfolding div_floor_nat_def by (metis div_le_mono div_mult_self1_is_m)
next
  fix m n :: nat
  assume "n < 0"
  thus "m \<le> n * div_floor m n" by simp
next
  fix m n k :: nat
  assume "n < 0"
  thus "k \<le> div_floor m n" by simp
qed

end (* div_semiring *)
  
instantiation nat :: number_dom_pre
begin
  
instance by (standard, simp, simp)

end
  
instantiation nat :: number_dom_min
begin

instance by (standard, rule)

end

subsection \<open>Type @{typ int}\<close>

instantiation int :: div_semiring
begin

definition div_ceil_int :: "int \<Rightarrow> int \<Rightarrow> int" where
  "div_ceil_int a b = \<lceil>rat_of_int a / rat_of_int b\<rceil>"
definition div_floor_int :: "int \<Rightarrow> int \<Rightarrow> int" where
  "div_floor_int a b = \<lfloor>rat_of_int a / rat_of_int b\<rfloor>"

lemma div_ceil_int_alt:
  assumes "b \<noteq> (0::int)"
  shows "div_ceil a b = - div_floor a (-b)"
  unfolding div_floor_int_def div_ceil_int_def by linarith

lemma div_floor_int_leq_pos:
  assumes "0 < (b::int)"
  shows "b * div_floor a b \<le> a"
proof -
  from assms have "0 < rat_of_int b" by linarith
  hence "rat_of_int \<lfloor>rat_of_int a / rat_of_int b\<rfloor> * rat_of_int b \<le> rat_of_int a"
    using floor_correct pos_le_divide_eq by blast
  thus ?thesis unfolding div_floor_int_def
    by (metis (no_types) mult_of_int_commute of_int_le_iff of_int_mult)
qed

lemma div_floor_int_max_pos:
  assumes "0 < b" and "b * c \<le> (a::int)"
  shows "c \<le> div_floor a b"
proof -
  from assms(1) have pos: "0 < rat_of_int b" by linarith
  from assms(2) have "(rat_of_int b) * (rat_of_int c) \<le> rat_of_int a"
    by (metis of_int_le_iff of_int_mult)
  with pos have "rat_of_int c \<le> rat_of_int a / rat_of_int b"
    by (simp add: mult.commute mult_imp_le_div_pos)
  thus ?thesis unfolding div_floor_int_def by (simp add: le_floor_iff)
qed

lemma div_floor_int_geq_neg:
  assumes "b < (0::int)"
  shows "a \<le> b * div_floor a b"
proof -
  from assms have "rat_of_int b < 0" by linarith
  hence "rat_of_int a \<le> rat_of_int \<lfloor>rat_of_int a / rat_of_int b\<rfloor> * rat_of_int b"
    using floor_correct neg_le_divide_eq by blast
  thus ?thesis unfolding div_floor_int_def
    by (metis (no_types) mult_of_int_commute of_int_le_iff of_int_mult)
qed

lemma div_floor_int_max_neg:
  assumes "b < 0" and "a \<le> b * (c::int)"
  shows "c \<le> div_floor a b"
proof -
  from assms(1) have neg: "rat_of_int b < 0" by linarith
  from assms(2) have "rat_of_int a \<le> (rat_of_int b) * (rat_of_int c)"
    by (metis of_int_le_iff of_int_mult)
  with neg have "rat_of_int c \<le> rat_of_int a / rat_of_int b"
    by (simp add: mult.commute neg_le_divide_eq)
  thus ?thesis unfolding div_floor_int_def by (simp add: le_floor_iff)
qed
...
instance proof
  fix a b :: int
  assume "0 < b"
  hence "b \<noteq> 0" and "-b < 0" by simp_all
  from div_floor_int_geq_neg[OF \<open>-b < 0\<close>, of a]
  show "a \<le> b * div_ceil a b" unfolding div_ceil_int_alt[OF \<open>b \<noteq> 0\<close>] by simp
next
  fix a b c :: int
  assume "0 < b"
  hence "b \<noteq> 0" and "-b < 0" by simp_all
  assume "a \<le> b * c"
  hence "a \<le> (-b) * (-c)" by simp
  from div_floor_int_max_neg[OF \<open>-b < 0\<close> \<open>a \<le> (-b) * (-c)\<close>]
  show "div_ceil a b \<le> c" unfolding div_ceil_int_alt[OF \<open>b \<noteq> 0\<close>] by simp
next
  fix a b :: int
  assume "b < 0"
  hence "b \<noteq> 0" and "0 < -b" by simp_all
  from div_floor_int_leq_pos[OF \<open>0 < -b\<close>, of a]
  show "b * div_ceil a b \<le> a" unfolding div_ceil_int_alt[OF \<open>b \<noteq> 0\<close>] by simp
next
  fix a b c :: int
  assume "b < 0"
  hence "b \<noteq> 0" and "0 < -b" by simp_all
  assume "b * c \<le> a"
  hence "(-b) * (-c) \<le> a" by simp
  from div_floor_int_max_pos[OF \<open>0 < -b\<close> \<open>(-b) * (-c) \<le> a\<close>]
  show "div_ceil a b \<le> c" unfolding div_ceil_int_alt[OF \<open>b \<noteq> 0\<close>] by simp
qed (fact div_floor_int_leq_pos, fact div_floor_int_max_pos, fact div_floor_int_geq_neg, fact div_floor_int_max_neg)

end (* div_semiring *)

instantiation int :: number_dom_pre
begin
  
instance by (standard, simp, fact mult_less_cancel_right_disj)
...
subsection \<open>Types @{typ rat} and @{typ real}\<close>

instantiation rat :: number_field
begin
  
definition div_ceil_rat :: "rat \<Rightarrow> rat \<Rightarrow> rat" where "div_ceil_rat = (op /)"
definition div_floor_rat :: "rat \<Rightarrow> rat \<Rightarrow> rat" where "div_floor_rat = (op /)"
  
instance by (standard, simp_all add: div_ceil_rat_def div_floor_rat_def mult_imp_div_pos_le
            neg_divide_le_eq mult_imp_le_div_pos neg_le_divide_eq ab_semigroup_mult_class.mult.commute)
  
end

instantiation real :: number_field
begin
  
definition div_ceil_real :: "real \<Rightarrow> real \<Rightarrow> real" where "div_ceil_real = (op /)"
definition div_floor_real :: "real \<Rightarrow> real \<Rightarrow> real" where "div_floor_real = (op /)"
  
instance by (standard, simp_all add: div_ceil_real_def div_floor_real_def mult_imp_div_pos_le
            neg_divide_le_eq mult_imp_le_div_pos neg_le_divide_eq ab_semigroup_mult_class.mult.commute)
