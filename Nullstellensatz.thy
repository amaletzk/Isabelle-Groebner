...
text \<open>We prove the geometric and the field-theoretic version of Hilbert's Nullstellensatz, i.e. the
  precise correspondence between algebraic varieties and radical ideals.\<close>
...
text \<open>@{const map_indets} is needed for proving the field-theoretic version of the Nullstellensatz.\<close>
...
lemma map_indets_inverseE:
  ...
  obtains g where "map_indets g \<circ> map_indets f = (id::_ \<Rightarrow> _ \<Rightarrow>\<^sub>0 'a::comm_semiring_1)"
proof -
...
(* TODO: Use "Types_To_Sets" to prove this lemma for arbitrary \<open>X\<close>. *)
...
lemma max_ideal_shapeD:
  assumes "finite X" and "F \<subseteq> P[X]"
    and "generates_max_ideal X (F::(('x::linorder \<Rightarrow>\<^sub>0 nat) \<Rightarrow>\<^sub>0 'a::alg_closed_field) set)"
...
next
  case False
  {
    text \<open>We define the type \<open>'y\<close> to be isomorphic to X.\<close>
    ...
    define le_y where "le_y = (\<lambda>x y::'y. rep x \<le> rep y)"
    ...
    have fin: "class.finite TYPE('y)"
    proof
      from assms(1) have "finite (abs ` X)" by (rule finite_imageI)
      thus "finite (UNIV::'y set)" by (simp only: y.Abs_image)
    qed
    moreover have lo: "class.linorder le_y (strict le_y)"
      by standard (simp_all add: le_y_def y.Rep_inject linear)
